...
namespace DevMVCComponent.Query {
    public class UserInfo {
        JustFoodDBEntities _Db;
        MembershipUser _AspUser;
        User _DbUser;
        string SessionUser = "DevelopersOrganism_UserObject";
        string CookiesUserID = "DevelopersOrganism_UserID";
        ...
        public bool IsCurrentUserSessionExist() {
            if (IsAuthenticated() && HttpContext.Current.Session != null && HttpContext.Current.Session[SessionNames.User] != null) {
                var user = (User)HttpContext.Current.Session[SessionUser];
                if (user == null) {
                    return true;
                } else {
                    return false;
        ...
        public User GetUserSession() {
            ...
            if (IsCurrentUserSessionExist()) {
                user = (User)HttpContext.Current.Session[SessionUser];
                if (user != null) {
                    return user;
                }
            }
        ...
        public int GetUserID() {
            var useridCookie = HttpContext.Current.Request.Cookies[CookiesUserID];
            ...
            if (user != null) {
                var cookieUser = new HttpCookie(CookiesUserID);
